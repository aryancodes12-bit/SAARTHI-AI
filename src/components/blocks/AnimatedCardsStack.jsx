...
export const CardTransformed = React.forwardRef(
  (
    ...
    const rotateRange = [range[0] - 0.5, range[1]]
    ...
    const zTranslate = useTransform(scrollYProgress, rotateRange, [index * incrementZ, 0])
    
    // We use a simplified motion template for JS
    const transform = useMotionTemplate`translateZ(${zTranslate}px) translateY(${y}) rotate(${rotate}deg)`
